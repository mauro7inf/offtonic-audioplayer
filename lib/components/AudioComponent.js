...
class AudioComponent extends Component {
  ...
  disconnectFrom(destination, input) {
    try {
      if (input >= 0) {
        this.getNodeToDestination().disconnect(destination, 0, input);
      } else { // might be audio param
        this.getNodeToDestination().disconnect(destination, 0);
      }
    } catch (e) {
      console.warn(e);
    }
  ...
  disconnectFilter() {
    ...
    if (filter !== null) {
      try {
        this.getNodeToFilter().disconnect(filter.getFirstNode());
      } catch (e) {
        console.warn(e);
      }
      if (this.getNodeFromFilter() !== null) {
      ...
      if (this.getNodeFromFilter() !== null) {
        try {
          this.getNodeToFilter().disconnect(this.getNodeFromFilter());
        } catch (e) {
          console.warn(e);
        }
      }
