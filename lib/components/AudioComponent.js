...
class AudioComponent extends Component {
  ...
  on() {
    if (this.node === null) {
      ...
      if (this.node !== null) {
        this.node.connect(this.player.ground);
      }
    }
  }
  ...
  off() {
    if (this.node !== null) {
      this.node.disconnect(this.player.ground);
    }
  ...
  connectAudioParam(propertyName) {
    ...
    this.connectComponentOrRefToAudioParam(rawValue, paramName, true);
  }
  ...
  disconnectAudioParam(propertyName) {
    let paramName = this.constructor.getParamName(propertyName);
    let rawValue = this.getProperty(propertyName, false);
    this.disconnectComponentOrRefFromAudioParam(rawValue, paramName, true);
  }
  ...
  connectInputIndex(propertyName) {
    ...
    this.connectComponentOrRefToInputIndex(rawValue, inputIndex);
  }
  ...
  disconnectInputIndex(propertyName) {
    let rawValue = this.getProperty(propertyName, false);
    let inputIndex = this.constructor.propertyDescriptors[propertyName].inputIndex;
    this.disconnectComponentOrRefFromInputIndex(rawValue, inputIndex);
  }
  ...
  disconnectFilter() {
    ...
    let filter = this.resolveReference(rawFilter);
    if (filter !== null && this.getNodeToFilter() !== null) {
      this.constructor.disconnectNodes(this.getNodeToFilter(), 0, filter.getFirstNode());
      if (this.getNodeFromFilter() !== null) {
