    ...
    if (!event.isDone && (!('afterEvents' in event) || event.afterEvents.length === 0)) {
      if ('action' in event) {
        ...
        if (typeof action === 'function') {
          ...
          this.constructor.o.createComponent(action, this.player, this.registry, this.tuning).perform(this);
        }
      } else if ('create' in event) {
        let eventIndexToCreate = event.create;
        this.events[eventIndexToCreate].action =
          this.constructor.o.createComponent(this.events[eventIndexToCreate].action, this.player, this.registry, this.tuning);
      }
      event.isDone = true;
      if ('id' in event) {
