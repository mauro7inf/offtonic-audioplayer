class Component {
  ...
  static create(properties, parent, player, registry, tuning) {
    ...
    if ('tuning' in properties && properties.tuning !== null && typeof properties.tuning === 'object') {
      if ('className' in properties.tuning) {
        let newTuning = this.o.createComponent(properties.tuning, parent, player, registry, tuning);
        newTuning.play();
    ...
    return (new constructor()).withParent(parent).withPlayer(player).withRegistry(registry).withTuning(tuningToUse, hasOwnTuning).withProperties(properties);
  }
    ...
    if (value !== null && typeof value === 'object' && 'className' in value) {
      ...
      for (let i = 0; i < value.length; i++) {
        if (value[i] !== null && typeof value[i] === 'object' && 'className' in value[i]) {
          createdArray.push(this.createComponent(value[i]));
  ...
  genericSetter(propertyName, value) {
    ...
    let descriptor = this.constructor.propertyDescriptors[propertyName];
    if ('passThrough' in descriptor && this[descriptor.passThrough] !== null) {
      this[descriptor.passThrough].setProperty(propertyName, value);
    }
  }
  ...
  genericCleaner(propertyName) {
    ...
    let descriptor = this.constructor.propertyDescriptors[propertyName];
    if ('passThrough' in descriptor && this[descriptor.passThrough] !== null) {
      this[descriptor.passThrough].cleanupProperty(propertyName);
    }
  }
  ...
  createComponent(properties) {
    return this.constructor.o.createComponent(properties, this, this.player, this.registry, this.tuning);
  }
