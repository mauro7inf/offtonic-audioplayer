class Component {
  ...
  static create(properties, player, registry, tuning) {
    ...
    if ('tuning' in properties && properties.tuning !=- null && typeof properties.tuning === 'object') {
      if ('className' in properties.tuning) {
        ...
        tuning = registry.get(properties.tuning.ref);
      } else { // assume it's an actual Tuning object
        tuning = properties.tuning;
      }
    }
    return (new constructor()).withPlayer(player).withRegistry(registry).withTuning(tuning, hasOwnTuning).withProperties(properties);
  }
  ...
  setProperty(propertyName, value) {
    ...
    if (value !== null && typeof value === 'object' && 'className' in value) {
      value = this.constructor.o.createComponent(value, this.player, this.registry, this.tuning);
      ...
      value = this.constructor.o.createComponent({
        ...
        value: value
      }, this.player, this.registry, this.tuning);
